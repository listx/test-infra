...
func (c *InRepoConfigCacheStore) Load(ctx context.Context) error {
	if c.path == "" {
		return fmt.Errorf("cannot load from an empty path")
	}

	reader, err := c.opener.Reader(ctx, c.path)
	if err != nil {
		if prowio.IsNotExist(err) {
			return nil
		}
		return fmt.Errorf("open %s: %v", c.path, err)
	}
...
func (c *InRepoConfigCacheStore) Save(ctx context.Context) error {
	if c.path == "" {
	...
	writer, err := c.opener.Writer(ctx, c.path)
	if err != nil {
...
func (c *InRepoConfigCache) Copy() map[CacheKey]ProwYAML {
	...
	for k, v := range c.GetResolvedEntries() {
		typed[k.(CacheKey)] = *(v.(*ProwYAML))
	}
